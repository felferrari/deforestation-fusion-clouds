...
parser.add_argument( # PRODES residual shapefile
    '-r', '--residual-shape',
    type = pathlib.Path,
    default = paths.PRODES_RESIDUAL_SHP,
    help = 'Path to PRODES residual deforestation shapefile (.shp)'
)
...
v_residual_def = ogr.Open(str(args.residual_shape))
l_residual_def = v_residual_def.GetLayer()
...
output = os.path.join(args.output_path, f'{general.LABEL_PREFIX}_{args.year}.tif')
...
l_residual_def.SetAttributeFilter(where_past)
gdal.RasterizeLayer(target_ds, [1], l_residual_def, burn_values=[2])

l_residual_def.SetAttributeFilter(where_ref)
gdal.RasterizeLayer(target_ds, [1], l_residual_def, burn_values=[2])
