...
with open(outfile, 'w') as sys.stdout:
    ...
    for im_0 in tqdm(range(general.N_IMAGES_YEAR), desc = 'Img 0'):
        for im_1 in tqdm(range(general.N_IMAGES_YEAR), desc = 'Img 1', leave = False):
            ...
            save_geotiff(str(args.base_image), os.path.join(visual_path, f'{general.PREDICTION_PREFIX}_avg_prob_{args.experiment}_{im_0}_{im_1}.tif'), average_pred_prob, dtype = 'float')
            ...
            opt_files_0 = [os.path.join(paths.PREPARED_PATH, fi) for fi in prep_files if fi.startswith(f'{general.OPT_PREFIX}_{year_0}')]
            cmap_files_0 = [os.path.join(paths.PREPARED_PATH, f'{general.CMAP_PREFIX}_{fi[13:]}') for fi in opt_files_0]
            cloud_map_0 = np.load(cmap_files_0[im_0])
            opt_files_1 = [os.path.join(paths.PREPARED_PATH, fi) for fi in prep_files if fi.startswith(f'{general.OPT_PREFIX}_{year_1}')]
            cmap_files_1 = [os.path.join(paths.PREPARED_PATH, f'{general.CMAP_PREFIX}_{fi[13:]}') for fi in opt_files_1]
            cloud_map_1 = np.load(cmap_files_1[im_1])

            max_cmap = np.maximum(cloud_map_0, cloud_map_1)
